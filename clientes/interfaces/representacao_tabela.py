...
class MetaConfig:
    """
    Objeto que contém as configurações de uma interface de tabela
    """
    tabela = None
    dbf = None

    def __init__(self):
        self.tabela = ConfigTabela()
        self.dbf = ConfigDbf()
...
class MetaInterfaceTabela(type):
    ...
    def _adicionar_metodos(nova_classe):
        ...
        def objeto_model(self):
            ...
            for coluna in config_tabela.mapa.keys():
                assert hasattr(objeto, coluna), ('Não existe coluna no model {model} '
                                                 'com nome {coluna}').format(model=self.model.__name__, coluna=coluna)
        ...
        def _chave_dbf(self):
            ...
            for coluna_chave in self._meta.tabela.chave_primaria:
                try:
                    valor_coluna = self.objeto_model.__dict__.get(coluna_chave)

                    if not valor_coluna:
                        colunas_incompletas.append(coluna_chave)
                        continue

                except KeyError:
                    raise AssertionError(('Chave primária "{coluna}" não encontrada entre as propriedades '
                                          'do model {model_classe}').format(coluna=coluna_chave,
                                                                            model_classe=nome_model))

                campo_coluna = list(filter(lambda campo: campo.column == coluna_chave, campos_model))
                ...
                assert len(campo_coluna) == 1, ('Há mais de uma coluna com nome {coluna} '
                                                'no model {model_classe}').format(coluna=coluna_chave,
                                                                                  model_classe=nome_model)
            ...
            if colunas_incompletas:
                vars_coluna_incompletas = (self._meta.tabela.mapa.get(coluna) for coluna in colunas_incompletas)

                raise AssertionError('Para montar a chave do dbf, é necessário que sejam setadas'
                                     ' as seguintes variáveis: {}'.format(', '.join(vars_coluna_incompletas)))
        ...
        def _where_dbf(self):
            ...
            for coluna_chave in self._meta.tabela.chave_primaria:
                ...
                clausulas_where.append("{coluna} = {valor}".format(coluna=coluna_chave,
                                                                   valor=valor_coluna_formatada))

            if colunas_incompletas:
                vars_coluna_incompletas = (self._meta.tabela.mapa.get(coluna) for coluna in colunas_incompletas)

                raise AssertionError('Para montar o where do dbf, é necessário que sejam setadas'
                                     ' as seguintes variáveis: {}'.format(', '.join(vars_coluna_incompletas)))
...
class RepresentacaoTabela(object):
    """
    Interface que representa uma tabela

    Para configurar como a interface se comporta, devem ser definidas as suas propriedades na classe Meta.

    OBS: Para informações detalhadas e exemplos, consulte generico/interfaces/representacao_tabela.md

    Propriedades disponíveis:

        - model_tabela(Model Django):
            Model Django que representa a tabela

        - mapa_tabela(dict):
            Dicionário que mapeia as colunas da tabela com seus respectivos atributos presentes na interface. Formato dos valores: nome_coluna: nome_atributo.

        - chave_primaria(str | tuple | list):
            Nomes das colunas que compoem a chave primária, em string, para chaves únicas compostas de uma coluna só, ou tupla/lista para chaves composta de mais de uma coluna.
            IMPORTANTE: A ordem em que as coluna são passadas importa. Será a ordem em que os seus valores serão inseridos na string de gravação/deleção do DBF.

        - indice_dbf(int):
            Indice usado no DBF

        - var_filial_dbf(str):
            NOME da Variável que será usada como filial no DBF.

        - var_codigo_dbf(str)
            NOME da Variável que será usada como código no DBF.

        - string_seq_socket(str):
            String usada para chamar a geração de sequência via socket

        - params_str_seq_socket(tuple | list | dict):
            Parâmetros de formatação da string de geração de sequência. Pode ser tupla/lista para argumentos posicionais ou dicionário para argumentos nomeados

        - gera_codigo_socket(bool):
            Se gera ou não sequência do código via socket ao instânciar a classe de interface
    """
